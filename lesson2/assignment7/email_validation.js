...
// between each component. Each component must contain one or more letters (A-Z,
// a-z) only.
...
function validLocal(local) {
  return !!local.match(/^[0-9a-z]+$/i);
}

function validDomain(domain) {
  return !!domain.match(/^[a-z]+(\.[a-z]+)+$/i);
}
...
function isValidEmail(email) {
  ...
  return !!(validLocal(localPart) && validDomain(domainPart));
}
